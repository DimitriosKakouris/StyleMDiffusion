...
from initno.utils.ptp_utils import AttendExciteAttnProcessor, AttentionStore
from initno.utils.attn_utils import fn_show_attention, fn_smoothing_func, fn_get_topk, fn_clean_mask, fn_get_otsu_mask
...
class StableDiffusionInitNOPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    ...
    def fn_augmented_compute_losss(
        ...
        # if sd_2_1:
        #     cross_attention_maps = aggregate_cross_attention_maps[:, :, 1:8]
        # else:
        ...
        if sd_2_1:
            if isinstance(prompt, list):
                ...
                print(f"Prompt 0: {prompt[0]}")

            last_idx = len(self.tokenizer(prompt).input_ids) - 1
            print(f"Last index: {last_idx}")
            cross_attention_maps = aggregate_cross_attention_maps[:, :, 1:last_idx]
        else:
            cross_attention_maps = aggregate_cross_attention_maps[:, :, 1:-1]
    ...
    def _perform_iterative_refinement_step(
        ...
        while cross_attn_loss > target_loss or self_attn_loss > target_self_loss:
            ...
            joint_loss, cross_attn_loss, clean_loss, self_attn_loss = self.fn_augmented_compute_losss(indices=indices, K=1, prompt = prompt)
        ...
        for iteration in tqdm(range(max_step)):
            ...
            for i, t in enumerate(timesteps):
                ...
                joint_loss, cross_attn_loss, self_attn_loss = self.fn_compute_loss(
                    indices=indices, K=1,prompt = prompt)
