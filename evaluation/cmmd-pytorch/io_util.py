...
class CMMDDataset(Dataset):
    ...
    def _get_image_list(self):
        ...
        for ext in ext_list:
         ...
         # Filter out images that match any of the ignore patterns
         #
        ignore_patterns = ["gen_seq"]
        filtered_image_list = []
        for image in image_list:
            if not any(pattern in image for pattern in ignore_patterns):
                filtered_image_list.append(image)

        # Sort the list to ensure a deterministic output.
        filtered_image_list.sort()
