...
def get_image_paths(path, sort=False):
    ...
    filtered_paths = [p for p in paths if not os.path.basename(p).startswith('gen_seq')]
...
def compute_fid(path_to_stylized, path_to_style, batch_size, device, num_workers=1):
    ...
    print(f"len stylized_image_paths: {len(stylized_image_paths)}")
    style_image_paths = get_image_paths(path_to_style)
    style_image_paths = style_image_paths * len(stylized_image_paths)
    
    print(f"len style_image_paths: {len(style_image_paths)}")
    ...
    # print(f"mu shape: {mu1.shape}, sigma shape: {sigma1.shape}")
...
def compute_art_fid(path_to_stylized, path_to_style, batch_size, device, mode='art_fid_inf', num_workers=1):
    """Computes the FID for the given paths.
    ...
    style_name = path_to_style.split('/')[-1]
    
    return fid_value.item(), style_name
...
def main():
    ...
    fid , style_name = compute_art_fid(args.tar,
                         args.sty,
    ...
    fid_str = f'FID of "{style_name}" : {fid:.4f}'
    print(fid_str)
    save_path = os.path.join(args.tar, f'fid_of_{style_name}.txt')
